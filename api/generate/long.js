...
const { generateLong } = require('../../services/geminiService');
...
// Generate long questions from uploaded PDF or image
module.exports = async (req, res) => {
  ...
  upload.single('file')(req, res, async (err) => {
    ...
    try {
      ...
      const questions = await generateLong(extractedText, parseInt(count), difficulty);

      res.status(200).json(successResponse('Long questions generated successfully', questions));
    } catch (error) {
